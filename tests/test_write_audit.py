...
UNAUDITED: dict[str, str] = {
    ...
    "master_data.py POST /sales-orders/{order_id}/reserve": "append-only ledger; the hold rows are the record",
    "master_data.py POST /sales-orders/{order_id}/release": "append-only ledger; the release rows are the record",
    ...
    "crm.py POST /campaigns/{campaign_id}/restore": "restore unaudited — known gap, the same as the other pipeline families",
    "activities.py POST /events/{event_id}/restore": "restore unaudited — known gap, the same as the other pipeline families",
