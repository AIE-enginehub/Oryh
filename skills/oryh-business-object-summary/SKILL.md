...
3. **Fetch the records** — in the same wave as step 2, the schema tells you how to READ them, not which to fetch; a headcount question ("who has not filed") needs no schema at all: `GET /business-objects?object_type=<type>&payload_match={"field":"value"}` for a payload-field filter (e.g. `{"department":"engineering"}`). `created_at_from` / `created_at_thru` (`YYYY-MM-DD`) narrow by filing date; `keyword=` searches title/summary/source_text, and `page`/`size` paginate (omit both to get every non-deleted match in one response). For a window on a date the payload carries, read the results and filter yourself. Say so if the result set looks too large to have summarized completely, rather than silently truncating.
