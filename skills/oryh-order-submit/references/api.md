...
GET /sales-orders/{order_id}/detail                     → order + items (each with `purchase_items`: 按单采购 lines behind it, with request status) + adjustments + linked quotation + computed_total + adjustments_total + adjusted_total + trail
```
...
POST /sales-orders
{
  ...
  "title": "市一医院 JC-800 订单",
  "quotation_id": "won-quotation-id",
  "customer_id": "customer-id",
  "contact_name": "设备科 周敏",
  "ship_to_address": "上海市黄浦区北京西路 100 号",
...
{"rows": [{
  ...
  "title": "2023年首批订单", "order_date": "2023-04-01", "contract_no": "HT-2023-088",
...
- **`status` accepts any state of the tenant's machine.** 已发货/已签收 documents
...
{"logistics_company": "顺丰速运", "logistics_tracking_no": "SF3021188299"}
```
