#!/usr/bin/env python3
"""Send normalised rows through a bulk upsert — master data or a stock take.
...
Usage (from this skill's directory):
    ...
    python3 scripts/bulk_import.py --kind inventory rows.json --expected-rows 1000
...
`--expected-rows N` is the number of data rows the SHEET has — read off its
last row number, not counted from what you extracted. A file reader that stops
at 1,000 lines hands you 999 rows and says nothing; a stock take of 999
positions out of 1,000 then lands cleanly and reports success. With the count
given, a shortfall is reported before anything is sent and the run stops.
Always pass it for a stock take; pass it for master data too when the sheet
is large enough for a reader to truncate.

Prints one JSON report on stdout:

    {"dry_run", "kind", "total_rows", "expected_rows", "chunks_sent", "chunks_applied",
...
SERVER_MAX_ROWS = 500
# kind → the bulk route. Master data lives at /{kind}/bulk; the stock take is
# the one whose path does not follow from its name.
ENDPOINTS = {
    "products": "/products/bulk",
    "vendors": "/vendors/bulk",
    "customers": "/customers/bulk",
    "inventory": "/inventory-items/bulk",
}
...
def _post_chunk(base_url: str, api_key: str, kind: str, payload: dict) -> dict:
    request = urllib.request.Request(
        f"{_site_origin(base_url)}/api/v1{ENDPOINTS[kind]}",
...
def main() -> int:
    ...
    parser.add_argument("--expected-rows", type=int, help="data rows the sheet has, from its last row number")
    ...
    if not isinstance(rows, list) or not rows:
        ...
        return 1
    if args.expected_rows is not None and args.expected_rows != len(rows):
        # Nothing is sent: a file that lost rows between the sheet and here is
        # the person's problem to see now, not a clean import to discover later.
        print(json.dumps({
            "dry_run": not args.apply, "kind": args.kind,
            "total_rows": len(rows), "expected_rows": args.expected_rows,
            "rows_missing_before_send": args.expected_rows - len(rows),
            "chunks_sent": 0, "chunks_applied": 0, "summary": {}, "changed_fields": {},
            "failures": [], "stopped_at_row": 0,
            "error": f"the sheet has {args.expected_rows} data rows but only {len(rows)} reached this "
                     f"script — re-read the sheet in parts (a file reader that stops at 1,000 lines "
                     f"is the usual cause) before importing anything",
