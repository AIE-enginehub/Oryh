...
| `inventory.manage` | `POST`/`PATCH`/`DELETE /inventory-items*`, `POST /inventory-items/bulk`, `POST /inventory-item-details` (reservations are `reserved` / `reservation_released` rows moving availability only); the picking run — `POST`/`PATCH`/`DELETE /picklists*`, `/picklist-items*`; also the freight legs — `POST`/`PATCH`/`DELETE /shipments*`, `/shipment-items*`, and `POST /shipments/{id}/post-stock` (the one once-only bridge from a shipment to the ledger, which consumes the order's hold as it moves the goods) — split out of `master_data.manage` so a warehouse keeper holds the stock ledger without holding the catalog; receiving against a PO stays with `purchase_order.manage` | `oryh-inventory` |
