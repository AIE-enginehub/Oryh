<!-- generated from docs/capabilities-skills-api.md by the help-references sync (sync_help_references) — edit the source, not this file -->
...
| `billing_account.post` *(scopable `:currency` / `:points`)* | `POST /business-objects/{id}/post-entries` (a tenant-defined account document whose type declares `account_effect: {reason, state}` posts its signed lines once; the poster needs the grant for each account's unit type), `POST /billing-accounts/{id}/expire` (the expiry sweep's write, held to the batch). **There is no direct entries write**: money moves through `POST /payments/{id}/apply`, opening balances through the account's creation | `oryh-billing-account` |
...
| `payroll.read` | **a read gate, not a write gate** — filters `GET /invoices*`, `GET /payments*`, `GET /payment-applications`, `GET /object-directory`, and gates `GET /pay-histories*`, `GET /employees/{id}/pay-history`. Also in the hosted flow agent's fixed grant set, so that payslips route through `oryh-invoice-approval-flow` like every other invoice | `oryh-payroll` |
...
| `master_data.manage` | `POST`/`PATCH`/`DELETE /projects`, `/vendors`, `/supplier-products`, `/customers`, `/customer-contacts`, `/customer-products`, `/products`, `/product-skus`, `/product-prices`, `/product-categories`, `/product-images`, `/bills-of-materials`, `/bom-items`, `/sales-channels`, `/stores`, `/store-facilities`, `/facilities`, `/resources`, `/external-product-maps` (which platform listing means which catalog product AT A GIVEN TIME — keyed by id or by verbatim title, bundles as rows, listing swaps as effective windows, `?at=` resolves by date; an order desk holding `order.submit_own` may POST a title-keyed row after a person confirms), plus `POST /products/bulk`, `/vendors/bulk`, `/customers/bulk`. Reads (`/product-matches` included) are member-visible | `oryh-master-data` |
| `purchase_order.manage` | `POST`/`PATCH`/`DELETE`/`restore` `/purchase-orders*` (returns to a vendor are rows with `order_kind: return`), `/purchase-order-items*`, `POST /purchase-orders/{id}/receive`; an order may name the purchase contract it executes | `oryh-purchase-order` |
| `contract.manage` *(scopable `:purchase` / `:sales`)* | `POST`/`PATCH`/`DELETE`/`restore` `/contracts*`, `/contract-items*`, `/contract-documents*` (the originals, any format), `/contract-terms*` (typed natural-language clauses); `GET /contracts/{id}/execution`. Reads are gated by the same capability, on the contract's side — a contract is read by the desk that files it | `oryh-contracts` |
| *(**`POST`/`DELETE /external-document-links`** is granted per linked document)* | `sales_order` → `order.submit_own`, `purchase_order` → `purchase_order.manage`, `invoice` → `invoice.manage:{direction}`, `payment` → `payment.record`, `business_object` → `business_object.write:{object_type}`, `inventory_item_detail` → `inventory.manage` (the row's writer) — a link is an annotation on the document, so whoever may record the document may say where it came from. Hard-unique per (source, kind, number, document); splits/merges are extra rows. Pinned by `tests/test_external_mapping.py` | `oryh-order-submit`, `oryh-inventory` |
| `inventory.manage` | `POST`/`PATCH`/`DELETE /inventory-items*`, `POST /inventory-items/bulk` (the count import), `POST /sales-orders/{id}/reserve` / `/release` (the hold pair, written by the server — `reserved` / `reservation_released` rows moving availability only), `POST /business-objects/{id}/post-stock` (a tenant-defined stock document whose type declares `stock_effect: {reason, state}` posts its signed lines once); the picking run — `POST`/`PATCH`/`DELETE /picklists*`, `/picklist-items*`. **`/inventory-item-details` is read-only**: every ledger row is written by a business act and carries it. Split out of `master_data.manage` so a warehouse keeper holds the stock ledger without holding the catalog; receiving against a PO stays with `purchase_order.manage`; includes `shipment.manage` | `oryh-inventory` |
| `shipment.manage` | the freight legs — `POST`/`PATCH`/`DELETE /shipments*`, `/shipment-items*`, and `POST /shipments/{id}/post-stock` (the one once-only bridge from a shipment to the ledger, which consumes the order's hold as it moves the goods). Split out of `inventory.manage` so the order desk that ships what it sold files and posts the parcel without reaching a position, a count or a stock document — the grant that makes "ship it" a shipment and nothing else. `inventory.manage` implies it | `oryh-shipping` |
| `fin_account.manage` | `GET`/`POST`/`PATCH`/`DELETE /fin-accounts*`, `GET`/`POST`/`PATCH /fin-account-transactions*` (the PATCH sets the reconciliation link only: one payment, or one batch of payments by their shared reference; bank charges are `fee` rows and never a payment), `POST /fin-account-transactions/bulk` — the treasury desk, reads included (cash and ledger split: cash position and payout lines are payroll-grade; the accountant's `payment.record` never reaches the register, and this capability never reaches payment documents). Deliberately granted to no shipped role — naming the cashier is the tenant's first treasury decision, and the setup report shows the gap | `oryh-treasury` |
...
| `keys.manage` | `GET`/`POST`/`PATCH /tenant/api-keys*`, `POST /users/{id}/skill-bundle`; also the tenant's side of flow driving — `GET /flow-runs`, `GET /flow-subscriptions` (or `flow_run.record`), `PATCH /flow-subscriptions/{id}` (`enabled` on/off, `clear_park` once a parked queue's cause is fixed) | `oryh-workspace-setup` |
...
| `skills.calibrate` (scopable: `skills.calibrate:oryh-product-matching`) | `GET /skills/{ref}` and `PATCH /skills/{ref}` with **only** `calibration` in the body — the workspace's own rules for one shipped skill (product-matching rules, report style). Files, audience and status stay with `skills.manage` | any skill whose calibration a desk maintains |
...
- A published `internal` policy is readable with **no capability at all**, and
  ...
  simply says `payroll.read`. See policies.md.
...
see scoped-skill-capabilities.md. Non-scopable
...
- scoped-skill-capabilities.md — walkthrough:
  a tenant-defined `daily_report` object type with its own scoped skill.
- ai-native-platform.md — the platform boundary
